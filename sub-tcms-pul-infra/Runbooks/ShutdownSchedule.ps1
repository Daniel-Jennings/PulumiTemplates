workflow ShutdownSchedule
{
    ...
    foreach($sub in $subs) {
        ...
        if($VerboseLogging) {
            Write-Output "**************************************************************************************************************";
            Write-Output "Running on subscription $($sub.Name)"
            Write-Output "**************************************************************************************************************";
        }
        ...
        $m = $allVms | measure;
        if($VerboseLogging) {
            Write-Output "Checking $($m.Count) Virtual Machines For 'ShutdownSchedule' Tag";
        }
        ...
        Foreach -Parallel ($vm in $allVms) {
            ...
            if($vm_tags.Count -gt 0) {
                ...
                if($WORKFLOW:VerboseLogging) {
                    Write-Output "$($vm.Name): No Tags Found";
                }
            }
        }
        ...
        Foreach -Parallel ($vmss in $allVmss) {
            ...
            if($vmss_tags.Count -gt 0) {
                ...
                if(!$matchingTagFound) {
                    ...
                    if($WORKFLOW:VerboseLogging) {
                        Write-Output "$($vmss.Name): No Matching Tags";
                    }
                }
            } else {
                $WORKFLOW:vmssNoTagCounter++;
                if($WORKFLOW:VerboseLogging) {
                    Write-Output "$($vmss.Name): No Tags Found";
                }
            }
        }
        ...
        InlineScript {
            ...
            Write-Output "**************************************************************************************************************";
            Write-Output "TASK SUMMARY: $USING:subName";
            Write-Output "";
            $formatted;
            Write-Output "**************************************************************************************************************";
        }
